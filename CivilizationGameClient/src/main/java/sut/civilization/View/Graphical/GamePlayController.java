...
//        inProgressTechnologyImage.setImage(new Image(Objects.requireNonNull(Civilization.class.getResourceAsStream(inProgressTechnology.imageAddress))));
    }

    private void getMapInfo() {
        Request request = new Request("game","getMap");
        ...
        Game.instance.map = (Land[][]) response.getDataToken("map");
        Game.instance.setPlayersInGame((ArrayList<User>) response.getDataToken("players"));
        Game.instance.setSubTurn((Integer) response.getDataToken("subTurn"));
        updateWholeMap();
    }
