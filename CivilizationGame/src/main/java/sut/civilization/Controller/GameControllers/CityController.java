...
public class CityController extends GameController {
        ...
        if (isCityBuildable(main)){
            ...
            for (int i = 0; i < 6; i++) {
                if (isValid(neighbors[i]))
                    Land.map[neighbors[i].x][neighbors[i].y].setOwnerCity(city);
            }
    ...
    public boolean isCityBuildable(Pair<Integer,Integer> main){
        if (!Land.map[main.x][main.y].getLandType().isWalkable)
            return false;
            ...
            if (isValid(neighbors[i])){
                ...
                for (int j = 0; j < 6; j++) {
                    if (isValid(neighbors2[j])){
                        if (Land.map[neighbors2[j].x][neighbors2[j].y].getOwnerCity() != null)
                            return false;
                    }
                }
            }
            ...
            if (isValid(neighbors[i])){
                ...
                for (int j = 0; j < 6; j++) {
                    if (Pair.isValid(neighbors2[j])){
                        if (Land.map[neighbors2[j].x][neighbors2[j].y].getOwnerCity() != null){
                            if (Land.map[neighbors2[j].x][neighbors2[j].y].getOwnerCity().equals(selectedCity)){
                                CombatUnit combatUnit = Land.map[x][y].getCombatUnit();
    ...
    public boolean isCitizenInRange(int x, int y){
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                neighbors2[j] = landController.getNeighborIndex(neighbors[i], j);
                if (selectedCity.getMainLand().equals(Land.map[neighbors2[j].x][neighbors2[j].y]))
                    return true;
            }
        }
        return false;
    ...
    public String cityBuyLand(Matcher matcher){
        ...
        for (int i = 0; i < 6; i++) {
            if (Pair.isValid(neighbors[i])){
                if (Land.map[neighbors[i].x][neighbors[i].y].getOwnerCity() != null && Land.map[neighbors[i].x][neighbors[i].y].getOwnerCity().equals(selectedCity)){
                    canBuy = true;
                    break;
                }
            }
        }
        Land land = Land.map[x][y];
